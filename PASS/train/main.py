...
from piwise.criterion import CrossEntropyLoss2d,FocalLoss2d,LovaszLoss2d,FocalTverskyLoss
...
def train(args, model, enc=False):
    ...
    if args.cuda:
        ...
        # criterion = FocalTverskyLoss(0.7,1) #tversky
        # criterion = FocalTverskyLoss(0.7,2) #focal tversky
        # criterion = Dice() #dice
    ...
    for epoch in range(start_epoch, args.num_epochs+1):
        ...
        if (doIouVal):
            ...
            print ("building: %.6f" % (iou_classes[1]*100.0), "%\t")
            print ("veget   : %.6f" % (iou_classes[2]*100.0), "%\t")
            print ("sky     : %.6f" % (iou_classes[3]*100.0), "%\t")
            print ("people  : %.6f" % (iou_classes[4]*100.0), "%\t")
            print ("vehicle : %.6f" % (iou_classes[5]*100.0), "%\t")
            print ("sidewalk: %.6f" % (iou_classes[6]*100.0), "%\t")
            ...
            print ("building: %.6f" % (acc_classes[1]*100.0), "%\t")
            print ("veget   : %.6f" % (acc_classes[2]*100.0), "%\t")
            print ("sky     : %.6f" % (acc_classes[3]*100.0), "%\t")
            print ("people  : %.6f" % (acc_classes[4]*100.0), "%\t")
            print ("vehicle : %.6f" % (acc_classes[5]*100.0), "%\t")
            print ("sidewalk: %.6f" % (acc_classes[6]*100.0), "%\t")
        ...
        if (doIouTrain):
          iouTrain, iou_classes, accTrain, acc_classes = iouEvalTrain.getIoU()
          ...
          accStr = getColorEntry(accTrain)+'{:0.2f}'.format(accTrain*100) + '\033[0m'
          print ("EPOCH ACC on TRAIN set: ", accStr, "%")
